...
function ReportModal(props) {
  ...
  const yesClose = async () => {
    const reportdata = {
      "reportCommentIndex": props.data.commentUserIndex,
      "reportDtype": "C",
...
const PostCommentMoreButton = (props) => {
  ...
  const deleteComment = async () => {
    await requestDelete(`${urls.comment}deleteCommentByCommentIndex/${props.data.commentIndex}`)
    handleClose();
  }
  return (
    <>
        ...
        <Fade in={open}>
          <Box sx={style}>
            ...
            <Typography id="transition-modal-description" sx={{ mt: 1 }}>
              {props.data.commentUserIndex === userStatus.userIndex ? (
                <Stack
                  direction="row"
                  justifyContent="flex-start"
                  alignItems="flex-start"
...
export default PostCommentMoreButton;
